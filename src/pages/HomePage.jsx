    ...
    <div className="min-h-screen bg-base-200">
      ...
      <div className="flex items-center justify-center min-h-[calc(100vh-4rem)] p-4">
        <div className="text-center max-w-2xl">
          ...
          <div className={`grid gap-4 mt-8 ${isAdmin() ? "grid-cols-1 md:grid-cols-3" : "grid-cols-1 md:grid-cols-2"}`}>
            {(isAdmin() || isTeacher()) && (
              <button
                onClick={() => setShowUploadModal(true)}
                className="card bg-base-100 shadow-xl hover:shadow-2xl transition-all duration-200 hover:scale-105 text-left"
              >
                <div className="card-body items-center text-center">
                  <div className="bg-accent/10 p-4 rounded-full mb-2">
                    <Mic className="h-8 w-8 text-accent" />
                  </div>
                  <h2 className="card-title">Upload Classroom Recording</h2>
                  <p className="text-base-content/60">
                    {isAdmin() ? "Upload classroom recording for a teacher" : "Upload classroom recording"}
                  </p>
                </div>
              </button>
            )}
