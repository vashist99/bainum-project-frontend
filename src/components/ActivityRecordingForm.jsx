 ...
 * - role="parent": saves to one selected linked child (home context)
 * - role="teacher": fans out to every supervised child (school context)
 */
export default function ActivityRecordingForm({
  role,
  variant = "page",
  linkedChildren = [],
  childId = "",
  ...
  const [reviewState, setReviewState] = useState(null); // { transcript, assessment, targetChildren, childId }
  ...
  function wrapShell(content) {
    ...
    return (
      <div className="card bg-base-100 shadow-xl border border-base-200">
        <div className="card-body p-4 sm:p-6">{content}</div>
      </div>
    );
  }
  ...
  const validateForUpload = () => {
    ...
    if (role === "parent" && !childId) {
      toast.error("Select a child to record for.");
      return false;
    }
    ...
    return wrapShell(
      <>
          ...
          <div className="alert alert-info mb-4 py-2 text-sm">
            <span>
              Review the transcript and per-category WPM. Accept to save
              {role === "parent" ? " for the selected child" : " for every linked child"},
              or reject to discard.
            </span>
          </div>
          <div className={`${isModal ? "modal-action" : "flex"} flex-col-reverse sm:flex-row gap-2 sm:gap-3 mt-4`}>
    ...
    <>
        ...
        <p className="text-sm text-base-content/70 mb-3">
          {role === "teacher"
            ...
            : "Choose a child, then record or upload audio from home."}
        </p>
        <div className="divider my-2" />
        ...
        {role === "parent" && (
          <div className="form-control w-full mb-3">
            <label className="label py-1">
              <span className="label-text font-semibold">Child</span>
              <span className="label-text-alt">Required</span>
            </label>
            {linkedChildren.length === 0 ? (
              <p className="text-sm text-base-content/60">
                No children linked yet. Accept an invitation before recording.
              </p>
            ) : (
              <select
                ...
                disabled={uploading || linkedChildren.length === 1}
              >
                {linkedChildren.length > 1 && (
                  <option value="">Select a child...</option>
                )}
                {linkedChildren.map((child) => (
                  <option key={child._id} value={String(child._id)}>
                    {child.name}
                  </option>
                ))}
              </select>
        ...
        <VettedLabelSelect
          ...
          customPlaceholder={
            context === "school"
              ...
              : "e.g., Backyard play, Car ride home"
          }
  ...
  return (
    <div className="card bg-base-100 shadow-xl border border-base-200">
      <div className="card-body p-4 sm:p-6 max-w-3xl">{formBody}</div>
    </div>
  );
}
